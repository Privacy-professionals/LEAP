(function () {
  ...
  function enhanceTrustPills() {
    const hero = byId("hero");
    if (!hero) return;
    const trustList = hero.querySelector(".pp-hero-copy ul");
    if (!trustList) return;
    trustList.classList.add("pp-trust-pills");
    trustList.querySelectorAll("li").forEach((item, index) => {
      item.classList.add("pp-trust-pill");
  ...
  function enhanceMotionTargets() {
    ...
    const certifications = root.querySelectorAll(".pp-certifications li");
    certifications.forEach((item, index) => {
      item.classList.add("pp-cert-logo");
      item.style.setProperty("--delay", `${index * 95}ms`);
    });
    const warning = root.querySelector(".pp-compliance-warning");
    if (warning) warning.classList.add("pp-motion-alert");
  }
  ...
  function addAmbientJourney() {
    const main = byId("main");
    const hero = byId("hero");
    if (!main || !hero || main.querySelector(".pp-ambient-journey")) return;
    const ambient = document.createElement("div");
    ambient.className = "pp-ambient-journey";
    ...
    ambient.innerHTML = `
      ...
      <span class="pp-flow-line" style="width:34%;inset-block-start:43%;inset-inline-start:35%;transform:rotate(122deg)"></span>
      <span class="pp-flow-line" style="width:45%;inset-block-start:63%;inset-inline-start:32%;transform:rotate(17deg)"></span>
      <div class="pp-css-core" style="opacity:.18;transform:scale(.62)">
        <div class="pp-core-orbit"></div>
        <div class="pp-core-shield"><img src="assets/hero-logo-core.png" alt=""></div>
      </div>
    `;
    hero.insertAdjacentElement("afterend", ambient);
  }
  ...
  function enhanceServices() {
    ...
    if (layout) {
      layout.classList.add("pp-services-layout");
      Array.from(layout.children).forEach((child) => {
        if (child.matches("div") && child.querySelector(":scope > article")) {
          Array.from(child.querySelectorAll(":scope > article")).forEach((article) => {
  ...
  function scrollState() {
    ...
    const serviceCards = Array.from(root.querySelectorAll(".pp-card"));
    const motionTargets = Array.from(root.querySelectorAll(".pp-compliance-warning, .pp-certifications li"));
    ...
    const update = () => {
      ...
      document.documentElement.style.setProperty("--pp-grid-y", `${Math.round(state.scroll * -120)}px`);
      document.documentElement.style.setProperty("--pp-grid-y-soft", `${Math.round(state.scroll * -54)}px`);
      ...
      let activeServiceDistance = Infinity;
      serviceCards.forEach((card) => {
        const rect = card.getBoundingClientRect();
        const visible = rect.top < vh * .88 && rect.bottom > vh * .08;
        const distance = Math.abs((rect.top + rect.height / 2) - vh / 2);
        if (visible && distance < activeServiceDistance) {
          activeServiceDistance = distance;
          activeServiceCard = card;
        }
        card.classList.toggle("is-visible", visible);
      });
      serviceCards.forEach((card) => card.classList.toggle("is-active", card === activeServiceCard));
